...
from app.utils import is_safe_select
...
if submitted and user_q.strip():
    ...
    if q.lower().startswith("select"):
        ...
        with st.spinner("Asking Genie..."):
            try:
                ...
                st.warning(str(ve)); st.stop()

    if not is_safe_select(sql_text):
        st.error("Only read-only single-statement SELECTs are allowed."); st.stop()
    ...
    if pdf.empty:
        st.info("No rows returned."); st.stop()
